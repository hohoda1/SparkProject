...
class BubblesScene: SIFloatingCollectionScene {
    ...
    func sortedFloatingNodes() -> [SIFloatingNode] {
        return floatingNodes.sorted { (node: SIFloatingNode, nextNode: SIFloatingNode) -> Bool in
            let distanceX = node.position.x.distance(to: magneticField.position.x)
            let distanceY = node.position.y.distance(to: magneticField.position.y)
            let nextDistanceX = nextNode.position.x.distance(to: magneticField.position.x)
            let nextDistanceY = nextNode.position.y.distance(to: magneticField.position.y)
            return distanceX * distanceX + distanceY * distanceY < nextDistanceX * nextDistanceX + nextDistanceY * nextDistanceY && node.state != .selected
        }
    }
